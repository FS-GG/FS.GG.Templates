...
So when you want the DECISION without the side effect, run **`batch --text -n 1`** (add `--include-backlog` to see past the column). `batch` is `next` uncapped: it makes no offer, takes no lock, and sits in the shim's `BOARD_READS`, so it still answers on a stale engine. `--text` is not optional in that spelling — `batch` defaults to **JSON**, where a drained board prints `[]` and the reasons go to stderr. With `--text` it makes exactly `next`'s decision minus the offer and answers in the same WORDS — the same "nothing schedulable right now." out of the one `nothingSchedulable` literal, the same per-item passed-over reasons and the same starved-queue banner out of the one `sayPassedOver` — so the wording cannot drift apart. **The STREAMS are not the same, and .github#1562 split them deliberately**: `batch --text` writes that headline to **stdout** (through `printChosen`), `next` writes it to **stderr**, because `next`'s stdout is a machine contract — one bare ref, read with `$(…)` — and a headline printed there handed `ref="$(fsgg-coord next --repo X)"` the *string* "nothing schedulable right now." at exit 0. Every other line was already on stderr for both, so #1562 moved one line and dropped none. The substitution advice above is therefore unchanged — same decision, same words, same reasons, same exit 0 — but do not CAPTURE the two verbs' stdout expecting the same bytes. That was never true on a NON-empty queue either: there `next` prints a bare ref, while `batch --text` prints a `schedulable in parallel (N):` header and an indented `  → <ref>` line per item. `tests/coord-engine-e2e/writes.sh` pins both halves against one board and one chore: `batch` leaves the chore lock's comment thread empty, `next` posts a marker naming its worker to it.
