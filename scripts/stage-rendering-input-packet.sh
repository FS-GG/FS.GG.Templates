...
[[ "$version" =~ ^0\.30\.0-svg-input\.1\.[0-9a-f]{8}(\.[0-9A-Za-z.-]+)?$ ]] || {
  echo "Rendering candidate version is outside the SVG-INPUT-01 family: $version" >&2
  exit 2
}
...
required={
 ...
 'FS.GG.UI.Scene.SvgBrowser':['fable/SvgInputHost.fsi']}
...
manifest={
    'schema':'fsgg.svg-input.rendering-candidate/v1',
...
echo "Rendering input packet: staged $version from $(git -C "$rendering" rev-parse HEAD)"
