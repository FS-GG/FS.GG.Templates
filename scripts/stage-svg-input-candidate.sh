...
[[ -f "$packet" ]] || { echo "SVG input candidate: packet manifest is missing" >&2; exit 2; }
[[ ! -e "$final_output" ]] || { echo "SVG input candidate: output identity already exists: $final_output" >&2; exit 2; }
...
if data.get('schema')!='fsgg.svg-input.rendering-candidate/v1': raise SystemExit('unsupported Rendering packet schema')
...
if not re.fullmatch(r'0\.30\.0-svg-input\.1\.[0-9a-f]{8}(\.[0-9A-Za-z.-]+)?',version): raise SystemExit('invalid Rendering candidate version')
...
python3 - "$work/templates/fs-gg-fable-game/SvgFoundation/SvgFoundation.fsproj" "$work/templates/fs-gg-fable-game/SvgFoundation/Studio/Studio.fsproj" "$rendering_version" <<'PY'
from pathlib import Path
import sys
root_project=Path(sys.argv[1]); studio_project=Path(sys.argv[2]); version=sys.argv[3]
...
PY
template_version="0.12.0-svg-input.1"
...
result={'schema':'fsgg.svg-input.template-candidate/v1','rendering':{'mergedRevision':sys.argv[4],'packetSha256':sha(source),'version':rendering['version'],'packages':rendering['packages'],'fableInterfaces':rendering['fableInterfaces'],'worker':rendering['worker'],'resources':rendering['resources'],'npmLocks':rendering['npmLocks'],'modelEvidence':rendering['modelEvidence']},'templates':{'sourceRevision':sys.argv[5],'package':{'file':template.name,'version':'0.12.0-svg-input.1','sha256':sha(template),'payloadSha256':payload}},'publicPins':{'templates':'0.12.0','rendering':'0.30.0'},'publication':False}
destination.write_text(json.dumps(result,indent=2)+'\n')
PY
mv "$output" "$final_output"
output=''
echo "SVG input candidate: staged exact Rendering archives and Templates candidate at $final_output"
