...
# Playwright's browser is a versioned test runtime, not an ambient machine dependency.
(cd Browser.Tests && npm ci && npx playwright install --only-shell chromium && npm test)
