...
The lanes are `Server`, `Web`, `Server.Tests`, `Web/tests`, and `Browser.Tests`. Test reporters can write TRX/JUnit files into `artifacts/test-results/` and import them with `fsgg-sdd evidence --from-test-report`; SDD remains the single lifecycle owner.
