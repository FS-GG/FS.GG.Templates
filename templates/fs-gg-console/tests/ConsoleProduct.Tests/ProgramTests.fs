...
let run arguments cancelled =
    ...
    let ports: Program.ConsolePorts =
        {
            WriteOut = output.Add
    ...
    let exitCode =
        Program.run ports source.Token arguments
        |> Async.AwaitTask
...
let tests =
    testList
        "console contract"
        [
            testCase "arguments are written to stdout"
            <| fun _ ->
            ...
            testCase "failure is written to stderr"
            <| fun _ ->
            ...
            testCase "pre-cancelled work shuts down cleanly"
            <| fun _ ->
            ...
            testCase "waiting work observes cancellation and shuts down"
            <| fun _ ->
