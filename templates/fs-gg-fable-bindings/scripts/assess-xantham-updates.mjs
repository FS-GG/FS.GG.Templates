...
const reviewedUpdates = JSON.parse(await readFile(resolve(root, "xantham/release-assessments.json"), "utf8"));
if (reviewedUpdates.schemaVersion !== 1 || !Array.isArray(reviewedUpdates.releases)) throw new Error("invalid Xantham release assessments");
const assessment = await assessUpdates(baseline, fetch, () => new Date(), reviewedUpdates.releases);
