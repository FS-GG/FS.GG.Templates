...
let atTick tick content =
    ...
    { content with
        Hazard =
            { content.Hazard with
                X = content.Hazard.X + float phase * cellWidth
            }
    }
...
let hazardContact content cell =
    overlaps (playerBounds cell) content.Hazard
...
let interact content cell status =
    ...
    if status.Outcome <> "playing" then
        status
...
let resolveMove content current target status =
    if status.Outcome <> "playing" then
        ...
        let motion =
            {
                ...
                Displacement =
                    {
                        X = cellX target - cellX current
                        Y = cellY target - cellY current
                    }
            }
        ...
        let colliders =
            [
                ...
                collider
                    ...
                    {
                        X = boundary.X - boundaryThickness
                        Y = boundary.Y - boundaryThickness
                        Width = boundaryThickness
                        Height = boundary.Height + boundaryThickness * 2.0
                    }
                collider
                    "boundary-right"
                    KinematicResponse.Slide
        ...
        let accepted =
            if exact result.Bounds.X requested.X && exact result.Bounds.Y requested.Y then
                target
            else
                current
