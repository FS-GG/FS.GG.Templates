module FableGameWorkspaceNamespace.ArcadeRules
...
[<RequireQualifiedAccess>]
type ArcadeCommand =
    | SetVelocity of Point
    | Interact
    | Restart

let initialState content =
    {
        Player = content.Spawn
        Velocity = { X = 0.0; Y = 0.0 }
        Health = content.InitialHealth
        Score = 0
        Collected = false
        Outcome = ArcadeOutcome.Playing
...
let advanceOnce state =
    if state.Outcome <> ArcadeOutcome.Playing then
        state
    else
        ...
        let collider id response bounds =
            {
                Id = id
                Shape = KinematicShape.AxisAlignedBox bounds
                Response = response
            }
        ...
        let colliders =
            [
                ...
                collider
                    "arcade-boundary-left"
                    KinematicResponse.Slide
                    {
                        X = -content.Width
                        Y = -content.Height
                        Width = content.Width
                        Height = content.Height * 3.0
                    }
                collider
                    "arcade-boundary-right"
                    KinematicResponse.Slide
                    {
                        X = content.Width
                        Y = -content.Height
        ...
        let contact =
            overlaps moved.Bounds movingHazard
            || (moved.Hits |> List.exists (fun hit -> hit.ColliderId = "arcade-hazard"))

        let health =
            if contact && not state.HazardContact then
                max 0 (state.Health - content.HazardDamage)
            else
                state.Health
...
let applyCommand command state =
    ...
    | ArcadeCommand.SetVelocity velocity ->
        { state with
            Velocity = velocity
            Revision = state.Revision + 1UL
        }
    | ArcadeCommand.Interact ->
        let near x y =
            abs (state.Player.X + 5.0 - x) <= 8.0 && abs (state.Player.Y + 5.0 - y) <= 8.0

        if
            not state.Collected
            && near state.Content.CollectibleX state.Content.CollectibleY
        then
            { state with
                Collected = true
                Score = state.Score + state.Content.CollectibleScore
                Revision = state.Revision + 1UL
            }
...
let compatibilityFor content =
    {
        ContractVersion = 1
        EngineId = "fs-gg.svg-continuous-arcade"
        EngineVersion = "1"
        ProfileId = content.ContentId
        SchemaId = content.Id
        SchemaVersion = 1
    }
...
let contractForDefinition content : SessionContract<unit, ArcadeState, ArcadeCommand, ArcadeState, ArcadeState> =
...

    {
        Initialize =
            fun initialization ->
                if
                    initialization.SessionId <> content.Id
                    || initialization.Compatibility <> compatibility
                then
                    Error
                        {
                            Code = "arcade.initialize.identity"
                            Message = "arcade session identity mismatch"
                        }
                else
                    Ok(initialState content)
        AdmitInput =
            fun input state ->
                if input.SessionId <> content.Id then
                    Error
                        {
                            Code = "arcade.input.session"
                            Message = "arcade input session mismatch"
                        }
                else
                    Ok(applyCommand input.Value state)
        Advance =
            fun value state ->
                if value.SessionId <> content.Id then
                    Error
                        {
                            Code = "arcade.advance.session"
                            Message = "arcade advance session mismatch"
                        }
                else
                    let mutable current = state

                    for _ in 1UL .. value.StepCount do
                        current <- advanceOnce current
        ...
        Restore =
            fun snapshot ->
                if snapshot.SessionId <> content.Id then
                    Error
                        {
                            Code = "arcade.snapshot.session"
                            Message = "arcade snapshot session mismatch"
                        }
                elif snapshot.Compatibility <> compatibility then
                    Error
                        {
                            Code = "arcade.snapshot.compatibility"
                            Message = "arcade snapshot compatibility mismatch"
                        }
                elif snapshot.Value.Content <> content then
                    Error
                        {
                            Code = "arcade.snapshot.content"
                            Message = "arcade snapshot content mismatch"
                        }
                else
                    Ok snapshot.Value
    }
...
let canonicalState state =
    ...
    let point (value: Point) =
        $"{canonicalFloat value.X},{canonicalFloat value.Y}"

    let rect (value: Rect) =
        $"{canonicalFloat value.X},{canonicalFloat value.Y},{canonicalFloat value.Width},{canonicalFloat value.Height}"
