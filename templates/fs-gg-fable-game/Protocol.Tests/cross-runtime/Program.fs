...
module Arcade = FableGameWorkspaceNamespace.ArcadeRules
module Tactical = FableGameWorkspaceNamespace.TacticalRules
#if CONTINUOUS_PLAYER
...
let private writeFile (path: string) (contents: string) : unit =
    System.IO.File.WriteAllText(path, contents)
#endif

let private canonicalBootstrapRequest: BootstrapV1.Request =
    { Version = 1; PlayerName = "Rogue" }
            ...
            RealtimeV1.SnapshotMessage
                {
                    ...
                    Players =
                        [
                            { PlayerId = "p-1"; Col = 1; Row = 1 }
                            { PlayerId = "p-2"; Col = 2; Row = 3 }
                        ]
                }
        )
    | "presence" ->
        Some(
            RealtimeV1.PresenceMessage
                {
                    Version = 1
                    PlayerId = "p-1"
                    Joined = true
...
let private canonicalRealtimeV2Case (name: string) : RealtimeV2.Message option =
    let snapshot: RealtimeV2.Snapshot =
        {
            ...
            Collected = true
            Outcome = "playing"
            ContentId = "continuous-arena/cross-runtime"
            ContentSchema = 2
            CollectibleX = 60.0
            CollectibleY = 25.0
            HazardX = 88.0
    ...
    | "sessionHello" ->
        Some(
            RealtimeV2.SessionHelloMessage
                {
                    Version = 2
                    SessionCapability = "cross-runtime-capability"
                }
...
let private canonicalRealtimeV3Case (name: string) : RealtimeV3.Message option =
    let snapshot: RealtimeV3.Snapshot =
        {
            ...
            Collected = true
            Outcome = "playing"
            ContentId = "continuous-arena/cross-runtime-authored-v3"
            ContentSchema = 3
            BoundaryX = 0.0
            BoundaryY = 0.0
            BoundaryWidth = 220.0
            BoundaryHeight = 120.0
            SpawnCol = 19
            SpawnRow = 11
            CollectibleX = 60.25
            CollectibleY = 25.75
    ...
    | "sessionHello" ->
        Some(
            RealtimeV3.SessionHelloMessage
                {
                    Version = 3
                    SessionCapability = "cross-runtime-capability"
                }
...
let private rejectedCases =
    [
        ...
        "wrong-field-type",
        """{"kind":"input","payload":{"version":1,"sequence":"not-a-number","targetCol":1,"targetRow":1}}"""
    ]

let private encodeArenaIntent =
    function
...
let private encodeArenaCommand =
    function
...
let private interactionBoundaryProof () =
    ...
    let stateBefore collectibleX =
        ...
        createWith content |> join "boundary-player" { Col = 0; Row = 0 }
...
let private gameplayIdentityProof () =
    let visualIds =
        [| "arena"; "collectible"; "hazard"; "goal"; "thin-wall"; "player" |]
    ...
    let diagnostic number =
        {
            ...
            Properties =
                [
                    {
                        Key = "adjacent"
                        Value = SvgScenePropertyValue.Number number
                    }
                    {
                        Key = "tiny"
                        Value = SvgScenePropertyValue.Number 1.2345678901234567e-120
                    }
                    {
                        Key = "large"
                        Value = SvgScenePropertyValue.Coordinate { X = 1.2e100; Y = -3.4e-100 }
                    }
                ]
        }
                ...
                Some
                    {
                        ...
                        Step = { X = 1.0000000000000002; Y = 1.2e-100 }
                    }
            Entities = baseMetadata.Entities @ [ diagnostic number ]
        }

    let hash value =
        gameplayContentHash value arenaDocument
    ...
    let reboundMetadata =
        { firstMetadata with
            Entities =
                ...
                |> List.map (fun entity ->
                    if entity.EntityId = "gameplay:hazard" then
                        { entity with
                            VisualElementId = Some "goal"
                        }
                    else
                        entity)
        }

    let first, adjacent, rebound =
        hash firstMetadata, hash adjacentMetadata, hash reboundMetadata

    if first = adjacent || first = rebound then
        failwith "gameplay metadata identity merged distinct accepted semantics"
    ...
    String.concat
        ...
        [
            ...
            rebound
            canonicalFloat 1.0
            canonicalFloat 1.0000000000000002
        ]
...
let private arenaModelCorrespondenceProof traceFile =
    ...
    let interactionContent =
        { baseline with
            SchemaVersion = 3
            ContentId = "continuous-arena/model-interaction"
            CollectibleX = 5.0
            CollectibleY = 5.0
            Goal =
                {
    ...
    let staleContent =
        { interactionContent with
            ContentId = "continuous-arena/model-stale"
        }
    ...
    let hazardContent =
        { baseline with
            SchemaVersion = 3
            ContentId = "continuous-arena/model-hazard"
            Hazard =
                {
                    X = 0.0
                    Y = 0.0
    ...
    if observations <> expected then
        ...
        let at values =
            values |> List.tryItem index |> Option.defaultValue "<missing>"

        let action = at actions

        failwith
            $"Quint/reducer first divergence at state {index} ({action}): expected {at expected}; actual {at observations}"
...
let private arcadeProof traceFile =
    ...
    let priorCompatibility =
        { ContinuousPlayer.compatibility with
            EngineVersion = "0"
            ProfileId = "generated-player/prior"
        }
    ...
    let project (state: Arcade.ArcadeState) =
        ...
        let outcome =
            ...
            | Arcade.ArcadeOutcome.Playing -> "playing"
            | Arcade.ArcadeOutcome.Won -> "won"
            | Arcade.ArcadeOutcome.Lost -> "lost"
    ...
    let omitRound (line: string) =
        ...
        let finish =
            if start < 0 then
                ...
                line.IndexOf('|', start + marker.Length)

        if start < 0 || finish < 0 then
            failwith $"arena model observation lacks round field: {line}"
    ...
    let baseContent: Arcade.ArcadeContent =
        {
            ...
            ContentId = "sha256:arcade-cross-runtime"
            Width = 220.0
            Height = 120.0
            Spawn =
                {
    ...
    let initial =
        contract.Initialize
            {
                SessionId = baseContent.Id
                Compatibility = Arcade.compatibilityFor baseContent
                Configuration = ()
            }
        |> Result.defaultWith (fun failure -> failwith failure.Code)
    ...
    let terminalRefused =
        Arcade.applyCommand (Arcade.ArcadeCommand.SetVelocity { X = 2.25; Y = 0.0 }) won

    let restartedAfterWin =
        Arcade.applyCommand Arcade.ArcadeCommand.Restart terminalRefused

    let hazardContent =
        { baseContent with
            ContentId = "sha256:arcade-hazard"
            Hazard = baseContent.Spawn
        }
    ...
    let entered =
        Arcade.advanceOnce
            { restartedAfterWin with
                Content = hazardContent
                HazardX = hazardContent.Hazard.X
            }
    ...
    if lost.Outcome <> Arcade.ArcadeOutcome.Lost then
        failwith "arcade lethal contact was not applied"
    ...
    let wrongSession =
        { contract.Snapshot initial with
            SessionId = "foreign-session"
        }
    ...
    if observations <> expected then
        ...
        failwith
            $"Quint/Arcade reducer first divergence at state {index} ({at actions}): expected {at expected}; actual {at observations}"

    String.concat "\n" observations

let private tacticalProof traceFile =
    let apply label operation state =
        operation state
        |> Result.defaultWith (fun issue -> failwith $"tactical {label} refused: {issue}")

    let definition: Tactical.Content =
        {
            Id = "tactical"
            ContentIdentity = "sha256:tactical-cross-runtime"
            Columns = 10
            Rows = 6
            FirstId = "unit:7"
            First = { Col = 1; Row = 2 }
    ...
    let reviewedSimulation =
        repeatedSimulation |> apply "review simulation" Tactical.review

    if
        repeatedSimulation.Room.Players.[definition.FirstId].Cell.Col
        <= simulated.Room.Players.[definition.FirstId].Cell.Col
    then
        failwith "tactical repeated simulator step did not advance the accepted room"

    let alternate =
        { definition with
            ContentIdentity = "sha256:tactical-alternate"
            Target = { Col = 3; Row = 4 }
        }

    let alternatePlan =
        Tactical.create alternate |> apply "alternate row/column plan" Tactical.plan

    if alternatePlan.Route |> List.tryLast <> Some alternate.Target then
        failwith "tactical alternate route missed its authored target"

    let blocked =
        { definition with
            ContentIdentity = "sha256:tactical-blocked"
            Target = definition.Second
        }
        ...
        { initial.Room with
            Players =
                ...
                |> Map.add
                    definition.FirstId
                    { alteredPlayer with
                        PlayerId = "foreign-unit"
                    }
        }

    match
        tacticalContract.Restore
            { snapshot with
                Value = alteredIdentityRoom
            }
    with
    | Error failure when failure.Code = "tactical.snapshot.content" -> ()
...
let private arenaProof () =
        ...
        contract.Initialize
            {
                ...
                Configuration = createWith content
            }
        |> Result.defaultWith (fun failure -> failwith failure.Code)
    ...
    let apply command =
        ...
        let input =
            {
                SessionId = "arena-1"
                InputId = "cross-runtime-arena"
                Sequence = sequence
                Value = command
            }

        state <-
            contract.AdmitInput input state
            |> Result.defaultWith (fun failure -> failwith failure.Code)

        recording <-
            ReplayRecorder.appendInput input (canonicalState state) recording
            |> Result.defaultWith (fun issues -> failwithf "%A" issues)

    apply (Command.Join("p", { Col = 0; Row = 0 }))
    apply (Command.Apply("p", Intent.Move { Col = 1; Row = 0 }))

    if state.Status.Health <> 2 then
        failwith "cross-runtime contact was not applied"

    apply (Command.Apply("p", Intent.Move { Col = 0; Row = 0 }))
    apply (Command.Apply("p", Intent.Interact))

    if not state.Status.Collected || state.Status.Score <> 100 then
        failwith "cross-runtime collectible was not applied"

    apply (Command.Apply("p", Intent.Move { Col = 1; Row = 0 }))
    apply (Command.Apply("p", Intent.Move { Col = 2; Row = 0 }))
    apply (Command.Apply("p", Intent.Interact))

    if state.Status.Outcome <> "won" then
        failwith "cross-runtime win was not applied"
    ...
    if
        ...
        || state.Round <> 2
        || state.Room.Players.["p"].Cell <> { Col = 0; Row = 0 }
    then
    ...
    recording <-
        ReplayRecorder.appendAdvance 20UL (canonicalState state) recording
        |> Result.defaultWith (fun issues -> failwithf "%A" issues)
...
let main _ =
    ...
    | [ "decode-bootstrap-request"; inFile ] ->
        ...
        | Ok value when value = canonicalBootstrapRequest ->
            printfn "OK"
            0
        | Ok value ->
            printfn "MISMATCH %A" value
            1
        | Error message ->
            printfn "REJECTED %s" message
            1
    ...
    | [ "decode-bootstrap-response"; inFile ] ->
        ...
        | Ok value when value = canonicalBootstrapResponse ->
            printfn "OK"
            0
        | Ok value ->
            printfn "MISMATCH %A" value
    ...
    | [ "encode-realtime-v2"; caseName; outFile ] ->
        ...
        | Some message ->
            writeFile outFile (RealtimeV2.encodeMessage message)
            printfn "WROTE"
            0
        | None ->
            printfn "UNKNOWN-CASE %s" caseName
    ...
    | [ "encode-realtime-v3"; caseName; outFile ] ->
        ...
        | Some message ->
            writeFile outFile (RealtimeV3.encodeMessage message)
            printfn "WROTE"
            0
        | None ->
            printfn "UNKNOWN-CASE %s" caseName
    ...
    | [ "decode-rejected-cases" ] ->
        let results =
            ...
            |> List.map (fun (name, json) ->
                ...
                | Error _ -> name, true
                | Ok value ->
                    printfn "UNEXPECTEDLY-ACCEPTED %s -> %A" name value
                    name, false)
