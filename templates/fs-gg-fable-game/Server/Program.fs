        ...
        task {
            if enabled then
                while not stoppingToken.IsCancellationRequested do
                    ...
                    let v1Players: RealtimeV1.PlayerSnapshot list =
                        players
                        |> List.map (fun (pid, col, row) -> { PlayerId = pid; Col = col; Row = row })

                    let v1: RealtimeV1.Snapshot =
                        {
                            Version = 1
                            Tick = tick
                            Players = v1Players
                    ...
                    do!
                        hub.Clients
                            .Group($"{RoomAuthority.RoomId}-v1")
                            .SendAsync("Message", RealtimeV1.encodeMessage (RealtimeV1.SnapshotMessage v1))

                    let v2Players: RealtimeV2.PlayerSnapshot list =
                        players
                        |> List.map (fun (pid, col, row) -> { PlayerId = pid; Col = col; Row = row })
                    ...
                    do!
                        hub.Clients
                            .Group($"{RoomAuthority.RoomId}-v2")
                            .SendAsync("Message", RealtimeV2.encodeMessage (RealtimeV2.SnapshotMessage v2))

                    let content = snapshot.Content

                    let v3Players: RealtimeV3.PlayerSnapshot list =
                        players
                        |> List.map (fun (pid, col, row) -> { PlayerId = pid; Col = col; Row = row })
                    ...
                    do!
                        hub.Clients
                            .Group($"{RoomAuthority.RoomId}-v3")
                            .SendAsync("Message", RealtimeV3.encodeMessage (RealtimeV3.SnapshotMessage v3))
        }
