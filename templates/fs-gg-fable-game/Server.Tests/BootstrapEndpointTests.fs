...
type BootstrapEndpointTests() =
    ...
    member _.``POST /api/bootstrap returns a decodable, versioned response assigning a distinct player and the arena's fixed dimensions``
        ...
        task {
            ...
            use content =
                new StringContent(BootstrapV1.encodeRequest request, Encoding.UTF8, "application/json")
        ...
        task {
            ...
            let call () =
                task {
                    ...
                    return
                        BootstrapV1.responseFromJson body
                        |> Result.map (fun r -> r.PlayerId, (r.SpawnCol, r.SpawnRow))
                }
    ...
    member _.``POST /api/bootstrap rejects a malformed request body``() =
        task {
            ...
            use content =
                new StringContent("""{"version":1}""", Encoding.UTF8, "application/json")
