...
type GameHubTests() =
    ...
    let nextRaw (connection: HubConnection) : Task<string> =
        ...
        let subscription =
            connection.On<string>("Message", fun json -> tcs.TrySetResult json |> ignore)
    ...
    member _.``the hub URL contains no player identity and a validated hello returns a full resync``() =
        task {
            ...
            | RealtimeV1.ResyncSnapshotMessage snapshot ->
                Assert.Contains(snapshot.Players, fun p -> p.PlayerId = response.PlayerId)
    ...
    member _.``unknown capability and mismatched protocol version are rejected before authority is granted``() =
        task {
            ...
            let unknown =
                RealtimeV1.encodeMessage (
                    RealtimeV1.SessionHelloMessage
                        {
                            Version = 1
                            SessionCapability = "not-issued"
                        }
                )

            let! unknownError =
                Assert.ThrowsAsync<HubException>(fun () -> connection.InvokeAsync("SendMessage", unknown))
            ...
            let! versionError =
                Assert.ThrowsAsync<HubException>(fun () -> connection.InvokeAsync("SendMessage", badVersion))
    ...
    member _.``resync rejects an inconsistent future cursor and accepts a reached frontier``() =
        task {
            ...
            let future =
                RealtimeV1.encodeMessage (
                    RealtimeV1.ResyncRequestMessage
                        {
                            Version = 1
                            LastKnownTick = Int32.MaxValue
                        }
                )

            let! rejected =
                Assert.ThrowsAsync<HubException>(fun () -> connection.InvokeAsync("SendMessage", future))
    ...
    member _.``input is not applied on hub arrival and commits at the next tick frontier``() =
        task {
            ...
            let _, col, row =
                committed.Players |> List.find (fun (id, _, _) -> id = response.PlayerId)
    ...
    member _.``a duplicate sequence cannot replace an already admitted frontier intent``() =
        task {
            ...
            let! refused =
                Assert.ThrowsAsync<HubException>(fun () -> connection.InvokeAsync("SendMessage", duplicate))
            ...
            let! committed =
                nextMatching connection (function
                    | RealtimeV1.SnapshotMessage snapshot ->
                        snapshot.Players
                        |> List.exists (fun p -> p.PlayerId = response.PlayerId && p.Col = 1 && p.Row = 0)
                    | _ -> false)
    ...
    member _.``authoritative arena owns collection win refusal and restart``() =
        ...
        for sequence in 1..7 do
            submit sequence "move" 5 2

        submit 8 "interact" 5 2
        Assert.Equal((3, 100, true, "playing"), RoomAuthority.gameStatus ())

        Assert.True(
            RoomAuthority.submitInput response.PlayerId "forged" 99 "interact" 5 2
            |> Result.isError
        )

        Assert.True(
            RoomAuthority.submitInput response.PlayerId response.SessionCapability 8 "interact" 5 2
            |> Result.isError
        )

        Assert.Equal((3, 100, true, "playing"), RoomAuthority.gameStatus ())

        for sequence in 9..22 do
            submit sequence "move" 16 5
    ...
    member _.``configured V2 content bytes drive the authority geometry``() =
        let json =
            """{"schemaVersion":2,"contentId":"continuous-arena/studio-edit","collectibleX":33,"collectibleY":44,"hazardX":55,"hazardY":66,"hazardWidth":11,"hazardHeight":10,"goalX":77,"goalY":88,"goalWidth":11,"goalHeight":10,"thinWallX":99,"thinWallY":1,"thinWallWidth":2,"thinWallHeight":48}"""
    ...
    member _.``configured V3 content preserves explicit boundary and authored spawn``() =
        let json =
            """{"schemaVersion":3,"contentId":"continuous-arena/studio-v3","boundaryX":0,"boundaryY":0,"boundaryWidth":220,"boundaryHeight":120,"spawnCol":2,"spawnRow":1,"collectibleX":33,"collectibleY":44,"hazardX":55,"hazardY":66,"hazardWidth":11,"hazardHeight":10,"goalX":77,"goalY":88,"goalWidth":11,"goalHeight":10,"thinWallX":99,"thinWallY":1,"thinWallWidth":2,"thinWallHeight":48}"""
    ...
    member _.``V3 hello and reconnect preserve authored immutable content and spawn``() =
        task {
            let json =
                """{"schemaVersion":3,"contentId":"continuous-arena/reconnect-v3","boundaryX":0,"boundaryY":0,"boundaryWidth":220,"boundaryHeight":120,"spawnCol":2,"spawnRow":1,"collectibleX":33,"collectibleY":44,"hazardX":55,"hazardY":66,"hazardWidth":11,"hazardHeight":10,"goalX":77,"goalY":88,"goalWidth":11,"goalHeight":10,"thinWallX":99,"thinWallY":1,"thinWallWidth":2,"thinWallHeight":48}"""
            ...
            let connectAndRead () =
                task {
                    ...
                    let waiting =
                        nextMatchingV3 connection (function
                            | RealtimeV3.ResyncSnapshotMessage _ -> true
                            | _ -> false)
            ...
            let assertSnapshot =
                ...
                | RealtimeV3.ResyncSnapshotMessage snapshot ->
                    Assert.Equal(
                        (0.0, 0.0, 220.0, 120.0),
                        (snapshot.BoundaryX, snapshot.BoundaryY, snapshot.BoundaryWidth, snapshot.BoundaryHeight)
                    )
                    ...
                    Assert.Contains(
                        snapshot.Players,
                        fun player -> player.PlayerId = response.PlayerId && player.Col = 2 && player.Row = 1
                    )
                | other -> Assert.Fail $"expected V3 resync, got {other}"
    ...
    member _.``V2 client refuses schema3 before consuming capability while V3 can bind``() =
        task {
            let json =
                """{"schemaVersion":3,"contentId":"continuous-arena/v3-only","boundaryX":0,"boundaryY":0,"boundaryWidth":220,"boundaryHeight":120,"spawnCol":19,"spawnRow":11,"collectibleX":33,"collectibleY":44,"hazardX":55,"hazardY":66,"hazardWidth":11,"hazardHeight":10,"goalX":77,"goalY":88,"goalWidth":11,"goalHeight":10,"thinWallX":99,"thinWallY":1,"thinWallWidth":2,"thinWallHeight":48}"""
            ...
            let! refused =
                Assert.ThrowsAsync<HubException>(fun () -> oldClient.InvokeAsync("SendMessage", oldHello))
