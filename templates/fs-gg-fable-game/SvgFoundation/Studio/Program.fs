...
let private container: HTMLElement =
    document.getElementById ("svg-authoring-studio")
...
do
    host.SetSelection [ "hazard" ]
    |> Result.defaultWith (fun error -> failwithf "%A" error)

do
    host.SetCamera retainedCamera
    |> Result.defaultWith (fun error -> failwithf "%A" error)

let mutable private playSourceHash = ""

let mutable private authoredContent =
    compileArenaContent state.Metadata state.Document |> Result.defaultWith failwith

let mutable private playState =
    ArenaRules.create ()
    |> ArenaRules.join "studio-player" ({ Col = 0; Row = 0 }: Cell)
...
let private updateInput observation =
    inputAdapter
    |> Option.iter (fun adapter -> adapter.Update observation |> ignore)

let private workspaceMode =
    function
...
let private updateWorkspace message =
    ...
    effects
    |> List.iter (function
        | SvgWorkspaceEffect.ActiveContextsChanged contexts ->
            updateInput (CommandResolverObservation.ContextsChanged contexts)
...
let private acceptCaptured gesture =
    ...
    let displacementOverrides =
        ...
        |> List.map (fun command ->
            let remaining =
                adapter.State.Profile.Bindings
                |> List.filter (fun binding -> binding.Command = command && binding.Gesture <> gesture)

            if remaining.IsEmpty then
                InputBindingOverride.UnbindCommand command
            else
                InputBindingOverride.ReplaceCommand(command, remaining))
    ...
    let candidate =
        { inputProfile with
            Overrides = inputProfile.Overrides @ displacementOverrides @ [ replacement ]
        }
    ...
    | Ok effective ->
        ...
        | element ->
            element.textContent <-
                $"Rebind command accepted. Conflict feedback: displaced {displaced.Length} command(s)."
...
let private handleInputEffect effect =
        ...
        | "workspace.palette" ->
            ...
            updateInput (
                CommandResolverObservation.PushModal
                    {
                        Context = "workspace.palette"
                        RestoreFocus = "generated-authoring-studio--scene"
                    }
            )
    ...
    | CommandResolverEffect.RequestFocus target ->
        if host.WorkspaceState.Overlay.IsSome then
            updateWorkspace SvgWorkspaceMessage.CloseOverlay
        else
...
do
    let effective =
        WorkspaceCommands.compile inputProfile
    ...
    for selector in
        [
            ...
            "#generated-authoring-studio--workspace-rebind"
        ] do
        ...
        | element ->
            element.addEventListener (
                ...
                fun _ ->
                    ...
                    updateInput (
                        CommandResolverObservation.ContextsChanged(SvgWorkspace.activeContexts host.WorkspaceState)
                    )

                    match host.WorkspaceState.Overlay with
                    | Some(SvgWorkspaceOverlay.CommandPalette restore) ->
                        updateInput (
                            CommandResolverObservation.PushModal
                                {
                                    Context = "workspace.palette"
                    ...
                    | Some(SvgWorkspaceOverlay.PossibleInputHelp restore) ->
                        updateInput (
                            CommandResolverObservation.PushModal
                                {
                                    Context = "workspace.help"
                    ...
                    | Some(SvgWorkspaceOverlay.RebindCommand(_, restore)) ->
                        updateInput (CommandResolverObservation.BeginCapture restore)
                    | None -> ()
...
let private hash (document: SvgDocument) =
    SvgAsset.contentHash document
    |> Result.defaultWith (fun issues -> failwithf "%A" issues)

let private transaction id operations =
    {
...
let private commit id operations =
    ...
    match
        host.Preview candidate
        |> Result.bind (fun () -> host.CommitGesture candidate.Id)
    with
    | Ok() ->
        state <- host.State
        announce id
...
let private startBlankGame () =
    ...
    replaceScene
        "Blank playable game started"
        emptyDocument
        {
            SceneId = emptyDocument.Id
            Layers = []
            Entities = []
            Grid = None
            ResourceReferences = []
    ...
    | Ok content ->
        ...
        playState <-
            ...
            |> ArenaRules.join
                "studio-player"
                ({
                    Col = content.Spawn.Col
                    Row = content.Spawn.Row
                }
                : Cell)
...
let private drawPlayableShapes () =
    replaceScene
        "Playable vector shapes drawn"
        authoredArenaDocument
        {
            SceneId = authoredArenaDocument.Id
            Layers = []
            Entities = []
            Grid = None
            ResourceReferences = []
...
let private assignGameplayRoles () =
    ...
    commit
        "Gameplay roles assigned"
        [
            SvgAuthoringOperation.ReplaceSceneMetadata(gameplayRoleMetadata authoredArenaDocument.Id ids)
        ]
...
let private defineGameplayRules () =
    ...
    commit
        "Interaction and win rules defined"
        [
            SvgAuthoringOperation.ReplaceSceneMetadata(gameplayMetadata authoredArenaDocument.Id ids)
        ]
...
let private rebindHazardAndGoalRoles () =
    ...
    | Ok previousContent ->
        ...
        let saved =
            (ArenaRules.contractForDefinition previousContent).Snapshot previousState
        ...
        let hazardVisual =
            ...
            |> List.tryFind (roleOf >> (=) (Some "hazard"))
            |> Option.bind _.VisualElementId

        let goalVisual =
            state.Metadata.Entities
            |> List.tryFind (roleOf >> (=) (Some "goal"))
            |> Option.bind _.VisualElementId
        ...
        | Some hazardId, Some goalId ->
                ...
                { state.Metadata with
                    Entities =
                        ...
                        |> List.map (fun entity ->
                            ...
                            | Some "hazard" ->
                                { entity with
                                    VisualElementId = Some goalId
                                }
                            | Some "goal" ->
                                { entity with
                                    VisualElementId = Some hazardId
                                }
                            | _ -> entity)
                }
            ...
            | Ok currentContent ->
                crossContentRestoreRefused <-
                    ...
                    && ((ArenaRules.contractForDefinition currentContent).Restore saved
                        |> Result.isError)

                authoredContent <- currentContent

                playState <-
                    ArenaRules.createWith currentContent
                    |> ArenaRules.join
                        "studio-player"
                        ({
                            Col = currentContent.Spawn.Col
                            Row = currentContent.Spawn.Row
                        }
                        : Cell)
            ...
            Rights =
                {
                    License = "CC0-1.0"
                    Attribution = None
                    Source = Some "generated neutral sample"
                }
            Dependencies = []
...
let private placeInstances () =
    match
        ...
        |> List.tryFind (fun asset -> asset.AssetId = "sample-object" && asset.Revision = 1)
    with
    ...
    | Some asset ->
        ...
        let overrides =
            ...
            |> Option.map (fun id ->
                [
                    {
                        ElementId = id
                        Property = SvgPrefabProperty.Visibility
                        Value = SvgPrefabOverrideValue.Visibility true
                    }
                ])
            |> Option.defaultValue []
        ...
        let entities =
            [
                {
                    EntityId = "sample-object-a"
                    KindId = "sample.object"
                    VisualElementId = None
                    PrefabInstanceId = Some first.InstanceId
                    Properties =
                        [
                            {
                                Key = "position"
                                Value = SvgScenePropertyValue.Coordinate { X = 24.5; Y = 30.25 }
                            }
                        ]
                }
                {
                    EntityId = "sample-object-b"
                    KindId = "sample.object"
                    VisualElementId = None
                    PrefabInstanceId = Some second.InstanceId
                    Properties =
                        [
                            {
                                Key = "position"
                                Value = SvgScenePropertyValue.Coordinate { X = 72.75; Y = 30.25 }
                            }
                        ]
                }
            ]
...
let private reviseAsset () =
    ...
    | Some previous ->
        let changed =
            { previous.Document with
                Children =
                    previous.Document.Children
                    |> List.skip (min 1 previous.Document.Children.Length)
            }
        ...
        commit
            "Asset revision conflict exposed"
            [
...
let private resolveConflicts () =
    let operations =
        state.Instances
        |> List.map (fun instance -> SvgAuthoringOperation.PutInstance { instance with Overrides = [] })

    if operations.IsEmpty then
        announce "Validation error: no instances"
    else
        commit "Asset conflicts resolved" operations
                    ...
                    { entity with
                        Properties =
                            ...
                            @ [
                                {
                                    Key = "label"
                                    Value = SvgScenePropertyValue.Text "edited"
                                }
                            ]
                    })
        }
...
let private authorGridAndFreeform () =
    ...
    |> continuousAdapter
    |> fun metadata ->
        commit "Grid and freeform adapters authored" [ SvgAuthoringOperation.ReplaceSceneMetadata metadata ]

let private refreshPlayableContent () =
    match compileArenaContent state.Metadata state.Document with
    | Ok content ->
        authoredContent <- content
        playState <- ArenaRules.withDefinition content playState
    | Error issue -> announce ("Validation error: " + issue)
...
let private movePlayerSpawn () =
    ...
    | Ok playerId ->
        commit
            "Player spawn moved to authored grid cell 2,1"
            [
                SvgAuthoringOperation.TransformElements(
                    [ playerId ],
                    SvgAffine.translate
                        (ArenaContent.cellX { Col = 2; Row = 1 })
                        (ArenaContent.cellY { Col = 2; Row = 1 })
                )
            ]
...
let private translateArenaUnsupported () =
    ...
    | Ok arenaId ->
        commit
            "Arena translated for validation"
            [
                SvgAuthoringOperation.TransformElements([ arenaId ], SvgAffine.translate 11.0 0.0)
            ]
...
let private movePlayableHazard label target =
    ...
    | Error issue, _
    | _, Error issue -> announce ("Validation error: " + issue)
    | Ok currentContent, Ok hazardId ->
        ...
        commit
            label
            [
                SvgAuthoringOperation.TransformElements([ hazardId ], SvgAffine.translate dx dy)
            ]
...
let private playEditedArenaStep () =
    ...
    match
        frozen
        |> Result.bind (fun (document, metadata) ->
            compileArenaContent metadata document
            |> Result.map (fun content -> document, metadata, content))
    with
    ...
    | Ok(frozenDocument, frozenMetadata, frozenContent) ->
        ...
        let target: ArenaContent.ArenaCell =
            {
                Col = min (ArenaContent.arenaColumns - 1) (currentCell.Col + 1)
                Row = currentCell.Row
            }
        ...
        let moveCommand =
            ArenaRules.Command.Apply("studio-player", ArenaRules.Intent.Move target)
        ...
        let roleId role =
            ...
            |> List.find (fun entity ->
                ...
                |> List.exists (fun property ->
                    property.Key = "role" && property.Value = SvgScenePropertyValue.Text role))
                    ...
                    |> List.map (fun element ->
                        if element.Id = playerId then
                            { element with
                                Transform =
                                    SvgAffine.translate
                                        (ArenaContent.cellX { Col = player.Col; Row = player.Row })
                                        (ArenaContent.cellY { Col = player.Col; Row = player.Row })
                            }
                        elif element.Id = hazardId then
                            let delta =
                                SvgAffine.translate
                                    (currentContent.Hazard.X - frozenContent.Hazard.X)
                                    (currentContent.Hazard.Y - frozenContent.Hazard.Y)

                            { element with
                                Transform = SvgAffine.compose delta element.Transform
                            }
                        else
                            element)
            }
        ...
        host.Preview
            {
                Schema = SvgAuthoring.transactionSchema
                Id = playTransactionId
                Operations = [ SvgAuthoringOperation.ReplaceDocument runtimeDocument ]
            }
        |> Result.defaultWith (fun error -> failwithf "%A" error)
...
let private validateRoundTrip () =
    ...
    | Ok restored when
        ...
        && restored.Catalog = state.Catalog
        && restored.Instances = state.Instances
...
let private invalidImportPayload kind =
    ...
    | "over-complex" ->
        let serialized =
            SvgScene.serialize (envelope state.Document)
            |> Result.defaultWith (fun issues -> failwithf "%A" issues)
    ...
    | "missing-reference" ->
        ...
        let document =
            ...
            | first :: remaining ->
                { state.Document with
                    Definitions = definition :: state.Document.Definitions
                    Children =
                        { first with
                            ClipId = Some definitionId
                        }
                        :: remaining
                }
        ...
        let serialized =
            SvgScene.serialize (envelope document)
            |> Result.defaultWith (fun issues -> failwithf "%A" issues)
        ...
        if at < 0 then
            failwith "missing-reference fixture was not encoded"

        serialized.Substring(0, at)
        + $"{missingId.Length}:{missingId}"
        + serialized.Substring(at + marker.Length)
...
let private storageKey =
    {
        Family = BrowserStorageFamily.ProjectDocument
        Slot = "continuous-arena"
    }
    ...
    | Ok serialized ->
        ...
        persistence.Persist(
            ...
            },
            {
                Key = storageKey
                SchemaVersion = 1
                PayloadHash = hash state.Document
                Payload = serialized
            }
        )
    | Error issues -> announce (sprintf "Validation error: %A" issues)
...
let private exerciseStorageFailure () =
    ...
    persistence.Persist(
        ...
        },
        {
            ...
            SchemaVersion = 1
            PayloadHash = hash state.Document
            Payload = String.replicate 262145 "x"
        }
    )
            ...
            JS.JSON.stringify (
                createObj
                    [
                        ...
                        "boundaryX" ==> content.Boundary.X
                        "boundaryY" ==> content.Boundary.Y
                        "boundaryWidth" ==> content.Boundary.Width
                        "boundaryHeight" ==> content.Boundary.Height
                        "spawnCol" ==> content.Spawn.Col
                        "spawnRow" ==> content.Spawn.Row
                        "collectibleX" ==> content.CollectibleX
                        "collectibleY" ==> content.CollectibleY
                        "hazardX" ==> content.Hazard.X
                        "hazardY" ==> content.Hazard.Y
                        "hazardWidth" ==> content.Hazard.Width
                        "hazardHeight" ==> content.Hazard.Height
                        "goalX" ==> content.Goal.X
                        "goalY" ==> content.Goal.Y
                        "goalWidth" ==> content.Goal.Width
                        "goalHeight" ==> content.Goal.Height
                        "thinWallX" ==> content.ThinWall.X
                        "thinWallY" ==> content.ThinWall.Y
                        "thinWallWidth" ==> content.ThinWall.Width
                        "thinWallHeight" ==> content.ThinWall.Height
                    ]
            )
        ...
        | Ok envelope when stored.SchemaVersion = 1 && stored.PayloadHash = hash envelope.Document ->
            let operations =
                [
                    ...
                    for asset in envelope.Catalog.Assets do
                        SvgAuthoringOperation.UpsertAsset asset
                    for instance in envelope.Instances do
...
let private verifyFont () =
    ...
    | Ok resource ->
        ...
        let document =
            { state.Document with
                Definitions = font :: state.Document.Definitions
                Children = state.Document.Children @ [ text ]
            }
        ...
        match
            ...
            |> Result.bind (
                SvgResourceInterchange.importXml
                    {
                        AssetNamespace = "generated-offline"
                        DocumentId = "generated-offline"
        ...
        | Error issues -> announce (sprintf "Validation error: %A" issues)
        | Ok restored when
            restored.Fonts = [ resource ]
            && restored.Document.Children.Length = document.Children.Length
            ->
            match SvgStudio.activateFont resource with
            | Ok activated ->
                announce "Verified Noto text exported and reopened offline"
                (activated :> IDisposable).Dispose()
            | Error issues -> announce (sprintf "Validation error: %A" issues)
...
let private booleanGeometry () =
    let path x =
        {
            Commands =
                [
                    PathCommand.MoveTo { X = x; Y = 8.0 }
                    PathCommand.LineTo { X = x + 18.0; Y = 8.0 }
                    PathCommand.LineTo { X = x + 18.0; Y = 26.0 }
                    PathCommand.LineTo { X = x; Y = 26.0 }
                    PathCommand.Close
                ]
    ...
    match
        SvgGeometry.prepare
            ("generated-boolean-" + string state.Revision)
            state.Revision
            PathOperation.Union
            [ path 8.0 ]
            [ path 16.0 ]
    ...
    | Ok prepared, Some worker ->
        match
            worker.Start(
                prepared,
                (fun result ->
                    match host.CommitGeometry(prepared, result) with
                    | Ok() ->
                        state <- host.State
                        announce "Boolean geometry committed once"
                    | Error error -> announce (sprintf "Validation error: %A" error)),
                (fun error -> announce ("Validation error: " + error))
            )
        with
...
[
    ...
    "Move playable hazard far away",
    fun () -> movePlayableHazard "Playable hazard moved far away" { Col = 18; Row = 10 }
    ...
    "Move playable hazard into next step",
    fun () ->
        let current = playState.Room.Players.["studio-player"].Cell

        movePlayableHazard
            "Playable hazard moved into next step"
            {
                Col = current.Col + 1
                Row = current.Row
            }
...
[
    ...
    fun () ->
        updateInput CommandResolverObservation.PopModal
        updateWorkspace SvgWorkspaceMessage.CloseOverlay
    ...
    fun () ->
        handleInputEffect (
            CommandResolverEffect.InvokeCommand
                {
                    EventId = "accessible-pointer"
                    Source = "pointer:control"
                    Command = "workspace.pointer"
                }
        )
    "Touch workspace action",
    fun () ->
        handleInputEffect (
            CommandResolverEffect.InvokeCommand
                {
                    EventId = "accessible-touch"
                    Source = "pointer:control"
                    Command = "workspace.touch"
                }
        )
    "Gamepad workspace action",
    fun () ->
        handleInputEffect (
            CommandResolverEffect.InvokeCommand
                {
                    EventId = "accessible-gamepad"
                    Source = "gamepad:accessible"
...
let private snapshot () =
    createObj
        [
            ...
            "assets" ==> state.Catalog.Assets.Length
            "instances" ==> state.Instances.Length
            "entities" ==> state.Metadata.Entities.Length
            "conflicts" ==> state.Conflicts.Length
            "schema" ==> SvgScene.schema
            "sceneId" ==> state.Metadata.SceneId
            "documentId" ==> state.Document.Id
            "contentHash" ==> hash state.Document
            "playSourceHash" ==> playSourceHash
            "exportedContentJson" ==> exportedContentJson
            "gameplayContentId"
            ==> (compileArenaContent state.Metadata state.Document
                 |> Result.map _.ContentId
                 |> Result.defaultValue "")
            "crossContentRestoreRefused" ==> crossContentRestoreRefused
            "viewBox"
            ==> $"{state.Document.ViewBox.X},{state.Document.ViewBox.Y},{state.Document.ViewBox.Width},{state.Document.ViewBox.Height}"
            "playHealth" ==> playState.Status.Health
            "playCollision" ==> playCollision
            "playScore" ==> playState.Status.Score
            "playCollected" ==> playState.Status.Collected
            "playOutcome" ==> playState.Status.Outcome
            "playPlayerCell"
            ==> (let cell = playState.Room.Players.["studio-player"].Cell in $"{cell.Col},{cell.Row}")
            "playCanonicalState" ==> ArenaRules.canonicalState playState
            "selectionCount" ==> host.Observe().SelectionCount
            "activePlayPreview" ==> (host.Observe().ActiveGesture = Some playTransactionId)
            "camera" ==> $"{retainedCamera.E},{retainedCamera.F}"
#if SVG_INPUT_CANDIDATE
            "workspaceMode" ==> workspaceMode host.WorkspaceState.Mode
            "workspaceOverlay"
            ==> (host.WorkspaceState.Overlay |> Option.map string |> Option.defaultValue "")
            "collapsedPanelCount"
            ==> (host.WorkspaceState.Layout.Panels
                 |> List.filter (fun panel -> panel.Effective = SvgPanelPlacement.Collapsed)
                 |> List.length)
...
expose (
    createObj
        [
            "snapshot" ==> snapshot
            "descriptorsValid"
            ==> (fun () -> SvgScene.validateDescriptors descriptors state.Metadata |> List.isEmpty)
