            ...
            rule "arena.collect" "Collect a nearby collectible" [] (fun state ->
                {
                    ...
                    Applies =
                        state.Status.Outcome = "playing"
                        && not state.Status.Collected
                        && (state.Room.Players
                            |> Map.exists (fun _ player ->
                                (interact
                                    state.Definition
                                    {
                                        Col = player.Cell.Col
                    ...
                    Causes =
                        [
                            {
                                Code = "public.collectible"
                                Message = state.Definition.ContentId
                            }
                        ]
            ...
            rule "arena.hazard" "Damage on hazard contact entry" [] (fun state ->
                {
                    ...
                    Causes =
                        [
                            {
                                Code = "public.health"
                                Message = string state.Status.Health
                            }
                        ]
...
let mount announce (getAcceptedState: unit -> State) =
    let timelinePanel, timelineOutput =
        addPanel "generated-replay-timeline" "Replay timeline"

    let inspectorPanel, inspectorOutput =
        addPanel "generated-replay-inspector" "Replay inspector"

    let plannerPanel, plannerOutput =
        addPanel "generated-scenario-planner" "Scenario planner"
    ...
    let mutable recording =
        ReplayRecorder.create (contract.Snapshot observedState) (canonicalState observedState)
        |> success "record"
    ...
    let resetObserved before =
        ...
        recording <-
            ReplayRecorder.create (contract.Snapshot before) (canonicalState before)
            |> success "record reset"

        sequence <- 0UL

    let observeInput before command after =
        if canonicalState before <> canonicalState observedState then
            resetObserved before
        ...
        recording <-
            ReplayRecorder.appendInput input (canonicalState after) recording
            |> success "append observed input"
    ...
    let observeAdvance before steps after =
        ...
        recording <-
            ReplayRecorder.appendAdvance steps (canonicalState after) recording
            |> success "append observed advance"
    ...
    addButton timelinePanel "Replay complete arena recording" (fun () ->
        ...
        | Ok(ReplayRunOutcome.Completed(next, state)) when canonicalState state = canonicalState observedState ->
            ...
            report
                timelineOutput
                "replay"
                $"Replayed {next} arena events; round {state.Round}; outcome {state.Status.Outcome}"
    ...
    addButton timelinePanel "Seek arena replay checkpoint" (fun () ->
        ...
        | Ok(ReplayRunOutcome.Completed(next, state)) ->
            report
                timelineOutput
                "seek"
                $"Seeked to {next}; collected {state.Status.Collected}; score {state.Status.Score}"
    ...
    addButton timelinePanel "Cancel arena replay safely" (fun () ->
        match
            Replay.seek
                ...
                canonicalState
                ((=) 2UL)
                (uint64 recording.Events.Length)
                { recording with Checkpoints = [] }
        with
        | Ok(ReplayRunOutcome.Cancelled(next, state)) ->
            report timelineOutput "cancel" $"Cancelled before event {next}; health {state.Status.Health}"
    ...
    addButton inspectorPanel "Diagnose arena replay divergence" (fun () ->
        let mutated =
            { recording with
                ...
                Events =
                    recording.Events
                    |> List.map (fun event ->
                        if event.Index = 2UL then
                            { event with StateDigest = "mutated" }
                        else
                            event)
            }
        ...
        | Ok(ReplayRunOutcome.Diverged divergence) ->
            report
                inspectorOutput
                "divergence"
                $"First divergence at event {divergence.EventIndex}: expected {divergence.ExpectedDigest}; actual {divergence.ActualDigest}"
    ...
    addButton plannerPanel "Branch and compare arena scenario" (fun () ->
        ...
        let mutable planning: PlanningSession<string, State, Intent> =
            Planning.create
                {
                    ContentId = basis.Definition.ContentId
                    Revision = uint64 basis.Room.Tick
                    Value = basis.Definition.ContentId
                }
                {
                    SessionId = "arena-1"
                    Revision = uint64 basis.Room.Tick
                    StateDigest = canonicalState basis
                    Value = basis
                }
            |> success "planning"
        ...
        let playerId =
            ...
            |> List.tryHead
            |> Option.map fst
            |> Option.defaultValue "studio-player"
        ...
        planning <-
            planning
            |> Planning.apply adapter "collect-next" Intent.Interact
            |> success "apply scenario"

        let comparison =
            Planning.compare "collect-next" planning |> success "compare scenario"
        ...
        report
            plannerOutput
            "plan"
            $"Accepted and predicted arena states compared at revision {comparison.BasisRevision}; scenario cancelled through Planning.cancel")

    addButton rulesPanel "Explain arena goal rule" (fun () ->
        let catalog = ruleCatalog ()

        let inspection =
            RuleCatalog.inspect "arena.goal" (getAcceptedState ()) catalog
            |> success "inspect rule"
