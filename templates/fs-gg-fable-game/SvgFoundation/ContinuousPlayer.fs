    ...
    [
        {
            ...
            Shape =
                KinematicShape.AxisAlignedBox
                    {
                        X = content.CollectibleX - 5.0
                        Y = content.CollectibleY - 5.0
                        Width = 10.0
                        Height = 10.0
                    }
...
let private advanceOnce state =
    if state.Outcome <> PlayerOutcome.Playing then
        state
    else
        let motion =
            {
        ...
        let collected =
            if Set.contains "collectible" ids then
                max 1 state.Collected
            else
                state.Collected

        let health =
            if Set.contains "hazard" ids then
                max 0 (state.Health - 1)
            else
                state.Health

        let score =
            if collected > state.Collected then
                state.Score + 100
            else
                state.Score
        ...
        let outcome =
            if health = 0 then
                PlayerOutcome.Lost
            elif Set.contains "goal" ids && collected > 0 then
                PlayerOutcome.Won
            else
...
let private applyCommand command state =
    if state.Outcome <> PlayerOutcome.Playing then
        ...
        | PlayerCommand.Collect ->
            ...
            if
                state.Collected = 0
                && near content.CollectibleX content.CollectibleY state.Player
            then
                { state with
                    Collected = 1
                ...
                state.Collected > 0
                && near
                    (content.Goal.X + content.Goal.Width / 2.0)
                    (content.Goal.Y + content.Goal.Height / 2.0)
                    state.Player
            then
                { state with
                    Outcome = PlayerOutcome.Won
                    Revision = state.Revision + 1UL
                }
            else
                { state with
        ...
        Restore =
            fun snapshot ->
                if snapshot.SessionId <> "generated-player" then
                    Error
                        {
                            Code = "continuous-player.snapshot.session"
                            Message = "continuous player snapshot session mismatch"
                        }
                elif snapshot.Compatibility <> compatibility then
                    Error
                        {
                            Code = "continuous-player.snapshot.compatibility"
                            Message = "continuous player snapshot compatibility mismatch"
                        }
                else
                    Ok snapshot.Value
    }
            ...
            [
                {
                    ...
                    Objects =
                        [
                            objectValue
                                ...
                                [
                                    SceneNode.Rectangle(
                                        (content.Boundary.X,
                                         content.Boundary.Y,
                                         content.Boundary.Width,
                                         content.Boundary.Height),
                                        color 241uy 245uy 249uy
                                    )
                                ]
                            objectValue
                                "thin-wall"
                                ...
                                [
                                    SceneNode.Rectangle(
                                        (content.ThinWall.X,
                                         content.ThinWall.Y,
                                         content.ThinWall.Width,
                                         content.ThinWall.Height),
                            ...
                            objectValue
                                "player"
                                $"Player, {outcome}, health {state.Health}, score {state.Score}"
                                true
                                [
                                    SceneNode.Rectangle(
                            ...
                            for id, col, row in peers do
                                objectValue
                                    ...
                                    [
                                        SceneNode.Rectangle(
                                            (float col * 11.0, float row * 10.0, 10.0, 10.0),
                                            color 124uy 58uy 237uy
                                        )
                                    ]
                        ]
                }
