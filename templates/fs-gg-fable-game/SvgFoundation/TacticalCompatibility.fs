module FableGameWorkspaceNamespace.TacticalCompatibility
...
let private sceneObject (value: DisclosedObject) =
    {
        ...
        Content =
            {
                Nodes = [ SceneNode.Circle(value.Center, value.Radius, value.Color) ]
            }
    }
...
let project (projection: DisclosedProjection) =
    ...
    let retainRelevant value =
        value |> Option.filter (fun id -> isVisibleSelectable id orderedLayers)
