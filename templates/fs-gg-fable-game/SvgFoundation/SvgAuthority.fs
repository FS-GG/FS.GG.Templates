...
let private accept message =
    ...
    | RealtimeV3.ResyncSnapshotMessage snapshot when
        snapshot.Version = 3
        && (snapshot.ContentSchema = 2 || snapshot.ContentSchema = 3)
        && snapshot.ContentId.StartsWith("continuous-arena/")
                    ...
                    |> List.map (fun player ->
                        {
                            ...
                            Col = player.Col
                            Row = player.Row
                            IsSelf = self = Some player.PlayerId
                        })
            }
        ...
        content <-
            {
                SchemaVersion = snapshot.ContentSchema
                ContentId = snapshot.ContentId
                Boundary =
                    {
                        X = snapshot.BoundaryX
                        Y = snapshot.BoundaryY
                        Width = snapshot.BoundaryWidth
                        Height = snapshot.BoundaryHeight
                    }
                Spawn =
                    {
                        Col = snapshot.SpawnCol
                        Row = snapshot.SpawnRow
                    }
                CollectibleX = snapshot.CollectibleX
                CollectibleY = snapshot.CollectibleY
                Hazard =
                    {
                        X = snapshot.HazardX
                        Y = snapshot.HazardY
                        Width = snapshot.HazardWidth
                        Height = snapshot.HazardHeight
                    }
                Goal =
                    {
                        X = snapshot.GoalX
                        Y = snapshot.GoalY
                        Width = snapshot.GoalWidth
                        Height = snapshot.GoalHeight
                    }
                ThinWall =
                    {
                        X = snapshot.ThinWallX
                        Y = snapshot.ThinWallY
                        Width = snapshot.ThinWallWidth
                        Height = snapshot.ThinWallHeight
                    }
            }
        ...
        | Some pending when
            pending.InvocationCompletedAtTick
            |> Option.exists (fun tick -> snapshot.Tick > tick)
            ->
            inFlightIntent <- None
            dispatchNext ()
        | _ -> ()
...
let private connect capability =
    ...
    thenBoth (connection.start ()) (fun () -> sendHello capability connection) (fun error ->
        notify ("authority error: " + string error))

let start onSnapshot =
    publish <- onSnapshot

    let request: BootstrapV1.Request =
        {
            Version = 1
            PlayerName = "SVG player"
        }
            ...
            try
                ...
                state <-
                    { state with
                        ...
                        Capability = Some response.SessionCapability
                    }

                notify "connecting"
                connect response.SessionCapability
            with error ->
                notify ("bootstrap failed: " + error.Message)
        }
    )
            ...
            Some
                {
                    Sequence = sequence
                    Intent = intent
                    InvocationCompletedAtTick = None
                }
            ...
            (fun _ ->
                ...
                | Some pending when pending.Sequence = sequence ->
                    inFlightIntent <-
                        Some
                            { pending with
                                InvocationCompletedAtTick = Some state.Tick
                            }
                | _ -> ())
            (fun error ->
                match inFlightIntent with
...
let move deltaCol deltaRow =
    enqueue (RelativeMove(deltaCol, deltaRow))

let command action = enqueue (Action action)
...
let dispose () =
    ...
    state.Connection
    |> Option.iter (fun connection -> thenBoth (connection.stop ()) ignore ignore)
