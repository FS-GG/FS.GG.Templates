                                ...
                                for column in 0..3 do
                                    ...
                                    yield
                                        objectValue
                                            ...
                                            [
                                                SceneNode.Rectangle(
                                                    (float column * 32.0, float row * 32.0, 30.0, 30.0),
                                                    color 235uy 240uy 248uy
                                                )
                                            ]
                        ]
                }
...
let tacticalCompatibilityScene =
    (characterizedProjection 41 "shared-scene:41" |> project).Scene
...
let continuousHost =
    mount
        "foundation-continuous-host"
        "Generated continuous SVG game"
        (ContinuousPlayer.scene 1 initialPlayerRuntime.Current)
#else
#if LEGACY_SVG_PREVIEW
let continuousHost =
    mount "foundation-continuous-host" "Neutral continuous-coordinate fixture" continuousScene
#else
let continuousHost =
    failwith "The production SVG player requires FsGgSvgRuntimeCandidate=true."
#endif
#endif
#if LEGACY_SVG_PREVIEW
...
requireTransition "tactical focus" (tacticalCompatibilityHost.Dispatch(RetainedInteractionMessage.FocusNext 41))
#endif
...
let mutable private sessionHost: SvgSessionHost<ContinuousPlayer.PlayerState> option =
    None
...
let private updatePlayer observation =
    let next, _ =
        SessionRuntime.update ContinuousPlayer.contract observation playerRuntime
...
let private onAudioEvent event =
    ...
    playerInputScope.setAttribute (
        "data-audio-capability-excluded",
        string (SvgAuthority.excludesCapability (string event))
    )
#endif
...
let private operationId (value: SaveOperationId) =
    {
        BrowserStorageOperationId.Generation = value.Generation
        Operation = value.Operation
    }

let private saveOperationId (value: BrowserStorageOperationId) =
    {
        SaveOperationId.Generation = value.Generation
        Operation = value.Operation
    }
        ...
        | AutosaveEffect.Persist(operation, payload) ->
            persistenceHost.Persist(
                operationId operation,
                {
                    Key =
                        {
                            Family = BrowserStorageFamily.GameSave
                            Slot = "primary"
                        }
                    SchemaVersion = 1
...
let private scheduleAutosave payload =
    ...
    window.setTimeout ((fun () -> observeAutosave (AutosaveObservation.Tick(now + 50UL))), 50)
    |> ignore
...
let private animationHost =
    new SvgAnimationHost(animationCallbacks, SvgAnimationHost.defaultConfig, playerRuntime.Current.Revision)
...
let private startPresentation outcome =
    ...
    let clip =
        if outcome then
            PresentationPlayer.outcomeClip
        else
            PresentationPlayer.movementClip
    ...
    animationHost.Start(
        {
            ...
            Importance =
                if outcome then
                    ...
                    Decorative SvgReducedMotionBehavior.Settle
        }
    )
...
handlePersistenceEvent <-
    ...
    | BrowserPersistenceEvent.Persisted operation ->
        observeAutosave (AutosaveObservation.Persisted(saveOperationId operation))
...
#if !LEGACY_SVG_PREVIEW
        playerInputScope.setAttribute (
            "data-archive-capability-excluded",
            string (SvgAuthority.excludesCapability archive)
        )
#endif
        persistenceOutput.textContent <- "Archive exported"
    | BrowserPersistenceEvent.Imported _ ->
        persistenceHost.Load(
            {
                Family = BrowserStorageFamily.GameSave
                Slot = "primary"
            }
        ...
        RequestProjection =
            fun generation ->
                ...
                |> Option.iter (fun host ->
                    host.CompleteProjection(generation, presentationRevision, playerRuntime.Current))
        ApplyProjection =
            fun revision projection ->
                requireTransition
                    "continuous projection"
                    (continuousHost.Dispatch(
                        RetainedInteractionMessage.ReplaceScene(
                            ContinuousPlayer.sceneWithPeers (int revision) projection authorityPeers
                        )
                    ))
...
let private playerSessionHost =
    new SvgSessionHost<ContinuousPlayer.PlayerState>(callbacks, SvgSessionHost.defaultConfig)
...
#if !LEGACY_SVG_PREVIEW
let private applyAuthority
    status
    (players: SvgAuthority.Player list)
    tick
    round
    health
    score
    ...
    contentId
    contentSchema
    (content: FableGameWorkspaceNamespace.ArenaContent.ArenaContent)
    hazardCol
    hazardRow
    =
    ...
    playerInputScope.setAttribute (
        ...
        players
        |> List.sortBy _.Id
        |> List.map (fun player -> $"{player.Id}:{player.Col},{player.Row}")
        |> String.concat ";"
    )
    ...
    | Some self ->
        ...
        authorityPeers <-
            players
            |> List.filter (fun player -> not player.IsSelf)
            |> List.map (fun player -> player.Id, player.Col, player.Row)

        let accepted =
            ContinuousPlayer.atAuthoritativeSnapshot
                self.Col
                self.Row
                health
                score
...
let private dispatchGameCommand command =
    ...
    | "game.pause" ->
        if playerSessionHost.Observe().Status = SvgSessionStatus.Running then
            playerSessionHost.Pause()
        else
            playerSessionHost.Resume()
...
addPresentationControl "foundation-load" "Load game" (fun () ->
    persistenceHost.Load(
        {
            Family = BrowserStorageFamily.GameSave
            Slot = "primary"
...
#if LEGACY_SVG_PREVIEW
addPresentationControl "foundation-fail-save" "Exercise failed autosave" (fun () ->
    scheduleAutosave (System.String('x', 4097)))
#endif
#endif
#else
#if LEGACY_SVG_PREVIEW
let private dispatchGameCommand command =
    ...
    let result =
        match command with
        | "game.focus-next" ->
            tacticalCompatibilityHost.Dispatch(RetainedInteractionMessage.FocusNext revision)
            |> Some
        | "game.focus-previous" ->
            tacticalCompatibilityHost.Dispatch(RetainedInteractionMessage.FocusPrevious revision)
            |> Some
        | "game.activate" ->
            tacticalCompatibilityHost.State.FocusedObjectId
            |> Option.map (fun id ->
                tacticalCompatibilityHost.Dispatch(RetainedInteractionMessage.Select(revision, id)))
        | _ -> None
...
let private playerInputHost =
    new SvgInputHost(
        ...
        (function
        | CommandResolverEffect.InvokeCommand value -> dispatchGameCommand value.Command
        | _ -> ()),
