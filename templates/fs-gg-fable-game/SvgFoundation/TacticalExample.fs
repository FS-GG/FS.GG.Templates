module FableGameWorkspaceNamespace.SvgFoundation.TacticalExample
...
module RuleModel = FableGameWorkspaceNamespace.SvgFoundation.TacticalRuleModel
module Control = FableGameWorkspaceNamespace.TacticalRules
...
let private seedSha256 =
    "fd3a16eedd57a312f456a0d9851a5d1d40e7af1aea3f621e66a2c37b25873cca"

type private TacticalEffect =
    | RouteAvailable
    | RouteBlocked
...
let private decodeSeed () =
...

    {
        ...
        Columns = integer parsed?grid?columns
        Rows = integer parsed?grid?rows
        CellWidth = unbox<float> parsed?grid?cellWidth
        CellHeight = unbox<float> parsed?grid?cellHeight
        FirstId = unbox<string> first?id
        First =
            {
                Col = integer first?col
                Row = integer first?row
            }
        SecondId = unbox<string> second?id
        Second =
            {
                Col = integer second?col
                Row = integer second?row
            }
        Target =
            {
                Col = integer parsed?planningTarget?col
                Row = integer parsed?planningTarget?row
            }
    }
            ...
            Metadata =
                {
                    Id = "tactical.route"
                    Version = 1
                    Title = "Find an authored-grid route"
                    Summary = "Plan from the selected unit to the authored target around accepted occupancy."
                    DependsOn = []
                }
            Evaluate =
                fun state ->
...

                    {
                        ...
                        Causes =
                            [
                                {
                                    Code = "public.unit"
                                    Message = seed.FirstId
                                }
                                {
                                    Code = "public.target"
                                    Message = $"{seed.Target.Col},{seed.Target.Row}"
                                }
                                {
                                    Code = "public.occupied"
                                    Message = seed.SecondId
                                }
                            ]
                                ...
                                for col in 0 .. seed.Columns - 1 do
                                    yield
                                        item
                                            ...
                                            [
                                                SceneNode.Rectangle(
                                                    (float col * seed.CellWidth,
                                                     float row * seed.CellHeight,
                                                     seed.CellWidth - 1.0,
                                                     seed.CellHeight - 1.0),
                    ...
                    Objects =
                        [
                            for cell in routeCells do
                                yield
                                    item
                                        $"route:{cell.Col}:{cell.Row}"
                                        "Planned route"
                                        false
                                        [
                                            SceneNode.Rectangle(
                                                (float cell.Col * seed.CellWidth,
                                                 float cell.Row * seed.CellHeight,
                                                 seed.CellWidth - 1.0,
                                                 seed.CellHeight - 1.0),
                                                color 147uy 197uy 253uy
                                            )
                                        ]
                        ]
                }
                        ...
                        |> List.map (fun (id, player) ->
                            item
                                ...
                                [
                                    SceneNode.Circle(
                                        {
                                            X = float player.Cell.Col * seed.CellWidth + seed.CellWidth / 2.0
                                            Y = float player.Cell.Row * seed.CellHeight + seed.CellHeight / 2.0
                                        },
                                        6.0,
                                        color 30uy 64uy 175uy
                                    )
                                ])
                }
            ]
...
let mount () =
    ...
    let definition: Control.Content =
        {
            Id = seed.Id
            ContentIdentity = $"{seed.Id}@sha256:{seedSha256}"
    ...
    let host =
        SvgBrowser.mount
            canvas
            {
                Width = 220.0
                Height = 140.0
                AccessibleLabel = "Tactical planning example"
                WheelZoomFactor = 1.1
            }
            (projection seed revision session.Room session.Route)
            ignore
    ...
    let render label =
        ...
        host.Dispatch(RetainedInteractionMessage.ReplaceScene(projection seed revision session.Room session.Route))
        |> ignore
    ...
    add "Cancel tactical scenario" (fun () ->
        ...
        | Ok cancelled ->
            ...
            root.setAttribute (
                "data-cancel-preserved-accepted",
                string (Control.digest definition session.Room = acceptedBefore)
            )
    ...
    add "Review tactical state" (fun () ->
        ...
        | Ok reviewed ->
            session <- reviewed
            render $"review:{Control.digest definition session.Room}")
    ...
    add "Explain tactical route rule" (fun () ->
        ...
        | Ok inspection ->
            ...
            root.setAttribute (
                "data-rule-causes",
                causes
                |> List.map (fun cause -> $"{cause.Code}:{cause.Message}")
                |> String.concat ";"
            )
            ...
            let explanations =
                inspection.Evaluations |> List.map _.Explanation |> String.concat " "
