/// Executable characterization for the selectable disclosed tactical example.
module FableGameWorkspaceNamespace.TacticalCompatibilityTests
...
let run () =
    ...
    let rejectedSelection =
        { baselineInput with
            Selection =
                {
                    SelectedObjectId = Some undisclosedFact
                    FocusedObjectId = Some undisclosedFact
                }
        }
        |> project
    ...
    require
        (rejectedSelection.Interaction.SelectedObjectId.IsNone
         && rejectedSelection.Interaction.FocusedObjectId.IsNone)
    ...
    let replacement =
        SvgRetained.update (RetainedInteractionMessage.ReplaceScene revised.Scene) baseline.Interaction
    ...
    require
        (accessible
         |> List.forall (fun (id, label) -> id <> undisclosedFact && not (label.Contains undisclosedFact)))
    ...
    require
        (baseline.Scene.Layers
         ...
         |> List.forall (fun value ->
             value.Id <> undisclosedFact
             && not (value.AccessibleLabel.Contains undisclosedFact)))
...
let main _ =
    ...
    printfn
        "tactical-compatibility: identities=passed layers=passed camera=passed selection-focus=passed disclosure=passed revisions=passed"
