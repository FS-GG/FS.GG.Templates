module FableGameWorkspaceNamespace.PreviewDocument

open FS.GG.UI.Scene
open FableGameWorkspaceNamespace.PreviewFont
...
let private maskChild id value =
    leaf
        ...
        None
        {
            Nodes = [ SceneNode.Rectangle((0.0, 0.0, 1.0, 1.0), value) ]
        }
...
let private gradient =
    {
        ...
        Stops =
            [
                {
                    Offset = 0.0
                    Color = color 29uy 78uy 216uy 255uy
                    StopOpacity = 1.0
                }
                {
                    Offset = 0.5
                    Color = color 20uy 184uy 166uy 255uy
                    StopOpacity = 0.85
                }
                {
                    Offset = 1.0
                    Color = color 249uy 115uy 22uy 255uy
                    StopOpacity = 1.0
                }
            ]
        InheritFrom = None
...
let document =
    ...
    let fractionalPath =
        { leaf
              "preview-fractional-route"
              (Some "semantic:fractional-route")
              {
                  Nodes = [ SceneNode.Path(hole, paint (color 29uy 78uy 216uy 255uy)) ]
              } with
            Transform =
                SvgAffine.compose
    ...
    let luminance =
        { leaf
              "preview-luminance"
              (Some "semantic:luminance")
              {
                  Nodes = [ SceneNode.Circle(point 139.25 25.5, 13.25, color 124uy 58uy 237uy 255uy) ]
              } with
...

    {
        ...
        Definitions =
            [
                {
                    Id = "preview-gradient"
                    Content = SvgDefinitionContent.Gradient gradient
                }
                {
                    Id = "preview-symbol"
                    Content = SvgDefinitionContent.Symbol(Some(rect 0.0 0.0 12.0 12.0), [ symbolChild ])
                }
                {
                    ...
                    Content =
                        SvgDefinitionContent.Clip(
                            ...
                            [ SvgClipShape.Rectangle(rect 0.0 0.0 40.0 30.0) ]
                        )
                }
                {
...
let verifyPortable () =
    ...
    if SvgDocument.serialize restored <> Ok serialized then
        failwith "Preview-A canonical document round trip drifted"
    ...
    for token in
        ...
        if not (exported.Contains token) then
            failwithf "Preview-A SVG export omitted %s" token

    let state = selectedState ()

    if state.SelectedSemanticId <> Some "semantic:fractional-route" then
        failwith "Preview-A selection drifted"
