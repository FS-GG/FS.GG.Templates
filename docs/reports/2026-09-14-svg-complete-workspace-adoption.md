...
The manifest owns generated product source, projects, dependency locks, conformance tests, player and Studio files, examples, formal models, product scripts, and the product CI workflow. An existing managed file is replaceable only when its normalized bytes match the same logical path from a named public baseline, or already match the candidate. A customized `Domain`, `Protocol`, `Server`, build, scene, or model path is a collision and refuses the whole transaction before a backup or workspace write. Symlinks in any managed path chain are also refused.

Lifecycle trees (`.fsgg`, `.agents`, `.claude`, `.specify`), owner guidance, tool-manifest additions, lifecycle-aware `.gitignore` entries, root documentation, and files outside the managed list are inventoried and preserved. Refreshing SDD owner guidance remains an SDD-owned operation; this transaction never copies or reattests it. Arbitrary gameplay AST, profile, evidence, save, replay, and keymap conversion is outside this byte-bounded adopter. Their existing readers or explicit compatibility refusals remain authoritative.
