...
- checklist: work/typed-sdd-p4-templates/checklist.md sha256:fbd5b34392e7d1c97de8dd1d2318d0858f0dc8ada488e38c895465ed5f3bad46 schemaVersion:1
...
## Plan Decisions
- PD-001 [AC-001] [FR-001] [DEC-001] complete: Add `lifecycle` choice symbols (`none`, `sdd`, `typed-sdd`, retained `spec-kit`, default `sdd`) to console, web, fable-game, and fable-bindings; declare the same optional parameter in all provider descriptors.
...
- PD-008 [AC-005] [FR-008] [DEC-003] complete: Generate the provider/profile/lifecycle matrix from descriptors and template profiles, including retained `spec-kit`, while retaining one full restore/build/test/publish path per identity.
- PD-009 [AC-002] [FR-009] complete: Retain direct-template lane coverage and product-skill manifest exactness; add explicit Standard SDD, Freeform, and legacy Spec Kit compatibility assertions.
...
- PC-002 [PD-001] dotnet template CLI: four owned workspace templates accept `--lifecycle none|sdd|typed-sdd|spec-kit`; the value is orchestration metadata and emits no template-owned SDD lifecycle tree.
...
- VO-001 [PD-001] [PD-002] [PC-001] [PC-002] semanticTest: Validate descriptor defaults, all four explicit template choice symbols including retained `spec-kit`, exact forwarding, and lane-specific path sets for every matrix cell.
...
- VO-005 [PD-008] [PD-009] compatibilityTest: Run full composition and all identity lanes, including direct `dotnet new`, Standard SDD, Freeform, legacy Spec Kit, and skill-manifest checks.
