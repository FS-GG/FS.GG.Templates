#!/usr/bin/env python3
"""Offline repeated required-field controls for both provider readers."""
...
cases = [
    ("valid live required field", WEB, True),
    ("repeated equal required", WEB.replace(NEEDLE, NEEDLE + "        required: false\n"), False),
    ("repeated conflicting required", WEB.replace(NEEDLE, NEEDLE + "        required: true\n"), False),
]
failures = []
for label, replacement, accepted in cases:
    with tempfile.TemporaryDirectory(prefix="fsc05-required-duplicate-") as folder:
        ...
        for owner, command in commands:
            ...
            if not accepted and (result.returncode == 0 or "repeated parameter field 'required'" not in combined):
                failures.append(f"{label}: {owner} admitted repeated required or wrong refusal: "
