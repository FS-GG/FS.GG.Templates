#!/usr/bin/env python3
"""Offline parameter indentation controls for both provider readers."""
...
cases = [
    ("valid live indentation", WEB, True),
    ("nested comment remains ignored", WEB.replace(NEEDLE, NEEDLE + "          # explanatory\n"), True),
    ("nested field below default", WEB.replace(NEEDLE, NEEDLE + "          nested: value\n"), False),
    ("misindented field", WEB.replace(NEEDLE, NEEDLE + "       extra: value\n"), False),
]
failures = []
for label, replacement, accepted in cases:
    with tempfile.TemporaryDirectory(prefix="fsc05-parameter-indent-") as folder:
        ...
        for owner, command in commands:
            ...
            if not accepted and (result.returncode == 0 or "unsupported parameter indentation" not in combined):
                failures.append(f"{label}: {owner} admitted invalid indentation or wrong refusal: "
