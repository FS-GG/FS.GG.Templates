...
trap cleanup EXIT
browser_window="$(xdotool search --sync --onlyvisible --class 'chromium|google-chrome' | tail -1)"
xdotool windowfocus --sync "$browser_window"
ORCA_PID="$orca_pid" BROWSER_PID="$browser_pid" /usr/bin/python3 "$script_dir/svg-input-orca.py" "$output"
