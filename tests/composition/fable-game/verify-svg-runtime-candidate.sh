...
game_packet="${2:?Game packet directory required}"
out="${3:?empty output directory required}"
[[ ! -e "$out" ]] || { echo 'svg-runtime-candidate: output exists' >&2; exit 2; }
bash "$root/scripts/stage-svg-runtime-candidate.sh" "$packet/manifest.json" "$game_packet/manifest.json" "$out"
candidate="$out/feed/FS.GG.Workspace.Template.0.12.0-svg-runtime.1.nupkg"
...
grep -F '>true</FsGgSvgRuntimeCandidate>' "$out/direct/SvgFoundation/SvgFoundation.fsproj" >/dev/null
grep -F "$game_version" "$out/direct/SvgFoundation/SvgFoundation.fsproj" >/dev/null
test -f "$out/direct/SvgFoundation/PlayerInput.fs"
test -f "$out/direct/SvgFoundation/ContinuousPlayer.fs"
...
cp "$root/tests/composition/fable-game/svg-runtime-player-observe.mjs" "$out/direct/Browser.Tests/"
...
for family in chromium firefox webkit; do
  ...
  (cd "$out/direct/Browser.Tests" && node svg-runtime-player-observe.mjs "$family" http://127.0.0.1:8140/) >"$out/$family-player.json"
done
...
jq -e '.result=="passed" and .process.name=="orca" and (.process.pid|type)=="number" and .composition=="generated-svg-studio" and .keyboard.events=="xdotool-X11" and .keyboard.selection=="passed" and .keyboard.focusRestoration=="passed" and .atspiActions.properties=="passed" and .atspiActions.validationFeedback=="passed" and .workspace.mode=="passed" and .workspace.palette=="passed" and .workspace.help=="passed" and .workspace.rebind=="passed" and .workspace.focusRestoration=="passed" and .screenReader.speechOutputObserved==true' "$SVG_SCENE_ORCA_OBSERVATION" >/dev/null
jq -n --slurpfile p "$out/candidate-packet.json" --slurpfile c "$out/chromium.json" --slurpfile f "$out/firefox.json" --slurpfile w "$out/webkit.json" --slurpfile cp "$out/chromium-player.json" --slurpfile fp "$out/firefox-player.json" --slurpfile wp "$out/webkit-player.json" --arg orca "$(sha256sum "$SVG_SCENE_ORCA_OBSERVATION"|cut -d' ' -f1)" '{schema:"fsgg.svg-runtime.template-qualification/v1",candidate:$p[0],routes:{direct:"passed",defaultUnselected:"passed",sdd17:{none:"passed",omittedDefault:"passed",typedProfile2:"passed"},wizard0111Adopter:"passed",retained011:"passed",retained010Staged:"passed"},browser:{chromium:$c[0],firefox:$f[0],webkit:$w[0],player:{chromium:$cp[0],firefox:$fp[0],webkit:$wp[0]},orcaSha256:$orca},authority:{fixedStep:"passed",collision:"passed",retainedProjection:"passed",pauseStepReset:"passed",winLoseRestart:"passed",keyboardPointerTouchGamepad:"passed"},adopter:{collision:"refused-without-write",interruption:"rolled-back",rollback:"byte-identical",provenance:"preserved"},contentMigration:{workspaceDistinct:true,newerSceneDowngrade:"unsupported-preserved"},publication:false}' >"$out/qualification.json"
echo "svg-runtime-candidate: passed; evidence=$out/qualification.json"
